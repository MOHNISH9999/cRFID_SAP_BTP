...
//    }
    ...
    void onEvent ( RFIDTriggerEvent rfidTriggerEvent ) {
        if ( rfidTriggerEvent.isPressed ( ) ) {
            ((SAPWizardApplication) getApplication ( )).getRFIDManager ( ).performInventory ( );
        } else {
            ((SAPWizardApplication) getApplication ( )).getRFIDManager ( ).stopInventory ( );
        }
    }
...
//    }
    ...
    void searchTag ( String tagID ) {
        ((SAPWizardApplication) getApplication ( )).getRFIDManager ( ).searchTag ( tagID );
    }
    ...
    void switchToSingleScanMode () {
        ((SAPWizardApplication) getApplication ( )).switchToSingleScanMode ( );
    }
    ...
    void disconnectHHReader () {
        ((SAPWizardApplication) getApplication ( )).disconnectHHReader ( );
    }
...
//    }
    ...
    void showMessage ( String title , String message , View.OnClickListener listener ) {
        ...
        SnackBarMaker.snackWithCustomTiming ( getWindow ( ).getDecorView ( ).getRootView ( ) ,
                message ,
                1000 );
    }
}
