package com.company.crfid_sap_btp.repository

import androidx.lifecycle.MutableLiveData

import com.company.crfid_sap_btp.archcomp.SingleLiveEvent
import com.company.crfid_sap_btp.mediaresource.EntityMediaResource
import com.company.crfid_sap_btp.repository.OperationResult.Operation
 ...
 * @param zCIMS_INT_SRV_Entities OData service
...
class Repository<T : EntityValue>(
        private val zCIMS_INT_SRV_Entities: ZCIMS_INT_SRV_Entities,
    ...
    init {
        ...
        if (EntityMediaResource.isV4(zCIMS_INT_SRV_Entities.metadata.versionCode) && EntityMediaResource.hasMediaResources(entitySet)) {
    ...
    fun read() {
        ...
        zCIMS_INT_SRV_Entities.executeQueryAsync(dataQuery,
            { result ->
    ...
    fun read(parent: EntityValue, navigationPropertyName: String) {
        ...
        zCIMS_INT_SRV_Entities.loadPropertyAsync(navigationProperty, parent, dataQuery,
            {
    ...
    fun create(newEntity: T, media: StreamBase) {
        if (newEntity.entityType.isMedia) {
            zCIMS_INT_SRV_Entities.createMediaAsync(newEntity, media,
                {
    ...
    fun create(newEntity: T) {
        ...
        zCIMS_INT_SRV_Entities.createEntityAsync(newEntity,
            {
                insertToCache(newEntity, entities)
                if (relatedEntities.size > 0) {
                    insertToCache(newEntity, relatedEntities)
    ...
    fun update(updateEntity: T) {
        zCIMS_INT_SRV_Entities.updateEntityAsync(updateEntity,
            {
    ...
    fun delete(deleteEntities: MutableList<T>) {
        ...
        zCIMS_INT_SRV_Entities.applyChangesAsync(deleteChangeSet,
            {
