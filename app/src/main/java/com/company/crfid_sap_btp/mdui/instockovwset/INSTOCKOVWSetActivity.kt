 ...
 * - [INSTOCKOVWSetCreateFragment]: to create or edit a(n) InStockOvw,
 * - [INSTOCKOVWSetDetailFragment]: to display the details of a(n) InStockOvw,
 * - [INSTOCKOVWSetListFragment]: to list INSTOCKOVWSet.
 *
 ...
 * what to do.
 */
class INSTOCKOVWSetActivity : AppCompatActivity(), InterfacedFragment.InterfacedFragmentListener<InStockOvw> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, INSTOCKOVWSetListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit()
        } else {
    ...
    override fun onFragmentStateChange(evt: Int, entity: InStockOvw?) {
        when (evt) {
    ...
    /**
     * Shows an AlertDialog when user wants to leave {@link INSTOCKOVWSetCreateFragment}.
     */
    ...
    private fun onItemClicked(entity: InStockOvw?) {
        if (!isMasterDetailView) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, INSTOCKOVWSetDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if (detail == null) {
                if (entity != null)
                    supportFragmentManager.beginTransaction()
                        .replace(R.id.detailFrame, INSTOCKOVWSetDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                        .commit()
            } else {
                if( entity == null) {
    ...
    private fun onCreateNewItem() {
        ...
        val fragment = INSTOCKOVWSetCreateFragment()
    ...
    @Suppress("UNUSED_PARAMETER")
    private fun onEditItem(entity: InStockOvw?) {
        val updateFragment = INSTOCKOVWSetCreateFragment()
        val arguments = Bundle()
    ...
    internal class ConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            builder.setPositiveButton(R.string.before_navigation_dialog_positive_button) { _, _ ->
                (fragmentActivity as INSTOCKOVWSetActivity).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null)
            }
    ...
    internal class DeleteConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            val fragmentActivity = requireActivity() as AppCompatActivity
            val builder = MaterialAlertDialogBuilder(fragmentActivity, R.style.AlertDialogStyle)
            val viewModel = ViewModelProvider(fragmentActivity)[InStockOvwViewModel::class.java]
