package com.company.crfid_sap_btp.mdui.inreprintrsnset
 ...
 * - [INREPRINTRSNSetCreateFragment]: to create or edit a(n) InReprintRsn,
 * - [INREPRINTRSNSetDetailFragment]: to display the details of a(n) InReprintRsn,
 * - [INREPRINTRSNSetListFragment]: to list INREPRINTRSNSet.
 *
 ...
 * what to do.
 */
class INREPRINTRSNSetActivity : AppCompatActivity(), InterfacedFragment.InterfacedFragmentListener<InReprintRsn> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, INREPRINTRSNSetListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit()
        } else {
    ...
    override fun onFragmentStateChange(evt: Int, entity: InReprintRsn?) {
        when (evt) {
    ...
    /**
     * Shows an AlertDialog when user wants to leave {@link INREPRINTRSNSetCreateFragment}.
     */
    ...
    private fun onItemClicked(entity: InReprintRsn?) {
        if (!isMasterDetailView) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, INREPRINTRSNSetDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if (detail == null) {
                if (entity != null)
                    supportFragmentManager.beginTransaction()
                        .replace(R.id.detailFrame, INREPRINTRSNSetDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                        .commit()
            } else {
                if( entity == null) {
    ...
    private fun onCreateNewItem() {
        ...
        val fragment = INREPRINTRSNSetCreateFragment()
    ...
    @Suppress("UNUSED_PARAMETER")
    private fun onEditItem(entity: InReprintRsn?) {
        val updateFragment = INREPRINTRSNSetCreateFragment()
        val arguments = Bundle()
    ...
    internal class ConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            builder.setPositiveButton(R.string.before_navigation_dialog_positive_button) { _, _ ->
                (fragmentActivity as INREPRINTRSNSetActivity).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null)
            }
    ...
    internal class DeleteConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            val fragmentActivity = requireActivity() as AppCompatActivity
            val builder = MaterialAlertDialogBuilder(fragmentActivity, R.style.AlertDialogStyle)
            val viewModel = ViewModelProvider(fragmentActivity)[InReprintRsnViewModel::class.java]
