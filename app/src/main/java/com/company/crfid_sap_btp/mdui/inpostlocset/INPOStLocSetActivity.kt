 ...
 * - [INPOStLocSetCreateFragment]: to create or edit a(n) INPOStLoc,
 * - [INPOStLocSetDetailFragment]: to display the details of a(n) INPOStLoc,
 * - [INPOStLocSetListFragment]: to list INPOStLocSet.
 *
 ...
 * what to do.
 */
class INPOStLocSetActivity : AppCompatActivity(), InterfacedFragment.InterfacedFragmentListener<INPOStLoc> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, INPOStLocSetListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit()
        } else {
    ...
    override fun onFragmentStateChange(evt: Int, entity: INPOStLoc?) {
        when (evt) {
    ...
    /**
     * Shows an AlertDialog when user wants to leave {@link INPOStLocSetCreateFragment}.
     */
    ...
    private fun onItemClicked(entity: INPOStLoc?) {
        if (!isMasterDetailView) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, INPOStLocSetDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if (detail == null) {
                if (entity != null)
                    supportFragmentManager.beginTransaction()
                        .replace(R.id.detailFrame, INPOStLocSetDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                        .commit()
            } else {
                if( entity == null) {
    ...
    private fun onCreateNewItem() {
        ...
        val fragment = INPOStLocSetCreateFragment()
    ...
    @Suppress("UNUSED_PARAMETER")
    private fun onEditItem(entity: INPOStLoc?) {
        val updateFragment = INPOStLocSetCreateFragment()
        val arguments = Bundle()
    ...
    internal class ConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            builder.setPositiveButton(R.string.before_navigation_dialog_positive_button) { _, _ ->
                (fragmentActivity as INPOStLocSetActivity).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null)
            }
    ...
    internal class DeleteConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            val fragmentActivity = requireActivity() as AppCompatActivity
            val builder = MaterialAlertDialogBuilder(fragmentActivity, R.style.AlertDialogStyle)
            val viewModel = ViewModelProvider(fragmentActivity)[INPOStLocViewModel::class.java]
