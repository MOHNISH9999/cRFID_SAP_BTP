 ...
 * - [INTMattomatSeSetCreateFragment]: to create or edit a(n) INTMattomatSe,
 * - [INTMattomatSeSetDetailFragment]: to display the details of a(n) INTMattomatSe,
 * - [INTMattomatSeSetListFragment]: to list INTMattomatSeSet.
 *
 ...
 * what to do.
 */
class INTMattomatSeSetActivity : AppCompatActivity(), InterfacedFragment.InterfacedFragmentListener<INTMattomatSe> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, INTMattomatSeSetListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit()
        } else {
    ...
    override fun onFragmentStateChange(evt: Int, entity: INTMattomatSe?) {
        when (evt) {
    ...
    /**
     * Shows an AlertDialog when user wants to leave {@link INTMattomatSeSetCreateFragment}.
     */
    ...
    private fun onItemClicked(entity: INTMattomatSe?) {
        if (!isMasterDetailView) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, INTMattomatSeSetDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if (detail == null) {
                if (entity != null)
                    supportFragmentManager.beginTransaction()
                        .replace(R.id.detailFrame, INTMattomatSeSetDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                        .commit()
            } else {
                if( entity == null) {
    ...
    private fun onCreateNewItem() {
        ...
        val fragment = INTMattomatSeSetCreateFragment()
    ...
    @Suppress("UNUSED_PARAMETER")
    private fun onEditItem(entity: INTMattomatSe?) {
        val updateFragment = INTMattomatSeSetCreateFragment()
        val arguments = Bundle()
    ...
    internal class ConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            builder.setPositiveButton(R.string.before_navigation_dialog_positive_button) { _, _ ->
                (fragmentActivity as INTMattomatSeSetActivity).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null)
            }
    ...
    internal class DeleteConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            val fragmentActivity = requireActivity() as AppCompatActivity
            val builder = MaterialAlertDialogBuilder(fragmentActivity, R.style.AlertDialogStyle)
            val viewModel = ViewModelProvider(fragmentActivity)[INTMattomatSeViewModel::class.java]
