 ...
 * - [LoginSrvSetCreateFragment]: to create or edit a(n) LoginSrv,
 * - [LoginSrvSetDetailFragment]: to display the details of a(n) LoginSrv,
 * - [LoginSrvSetListFragment]: to list LoginSrvSet.
 *
 ...
 * what to do.
 */
class LoginSrvSetActivity : AppCompatActivity(), InterfacedFragment.InterfacedFragmentListener<LoginSrv> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, LoginSrvSetListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit()
        } else {
    ...
    override fun onFragmentStateChange(evt: Int, entity: LoginSrv?) {
        when (evt) {
    ...
    /**
     * Shows an AlertDialog when user wants to leave {@link LoginSrvSetCreateFragment}.
     */
    ...
    private fun onItemClicked(entity: LoginSrv?) {
        if (!isMasterDetailView) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, LoginSrvSetDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if (detail == null) {
                if (entity != null)
                    supportFragmentManager.beginTransaction()
                        .replace(R.id.detailFrame, LoginSrvSetDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                        .commit()
            } else {
                if( entity == null) {
    ...
    private fun onCreateNewItem() {
        ...
        val fragment = LoginSrvSetCreateFragment()
    ...
    @Suppress("UNUSED_PARAMETER")
    private fun onEditItem(entity: LoginSrv?) {
        val updateFragment = LoginSrvSetCreateFragment()
        val arguments = Bundle()
    ...
    internal class ConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            builder.setPositiveButton(R.string.before_navigation_dialog_positive_button) { _, _ ->
                (fragmentActivity as LoginSrvSetActivity).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null)
            }
    ...
    internal class DeleteConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            val fragmentActivity = requireActivity() as AppCompatActivity
            val builder = MaterialAlertDialogBuilder(fragmentActivity, R.style.AlertDialogStyle)
            val viewModel = ViewModelProvider(fragmentActivity)[LoginSrvViewModel::class.java]
