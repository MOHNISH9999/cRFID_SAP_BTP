 ...
 * - [INREPRINTPOSetCreateFragment]: to create or edit a(n) InReprintPo,
 * - [INREPRINTPOSetDetailFragment]: to display the details of a(n) InReprintPo,
 * - [INREPRINTPOSetListFragment]: to list INREPRINTPOSet.
 *
 ...
 * what to do.
 */
class INREPRINTPOSetActivity : AppCompatActivity(), InterfacedFragment.InterfacedFragmentListener<InReprintPo> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, INREPRINTPOSetListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit()
        } else {
    ...
    override fun onFragmentStateChange(evt: Int, entity: InReprintPo?) {
        when (evt) {
    ...
    /**
     * Shows an AlertDialog when user wants to leave {@link INREPRINTPOSetCreateFragment}.
     */
    ...
    private fun onItemClicked(entity: InReprintPo?) {
        if (!isMasterDetailView) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, INREPRINTPOSetDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if (detail == null) {
                if (entity != null)
                    supportFragmentManager.beginTransaction()
                        .replace(R.id.detailFrame, INREPRINTPOSetDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                        .commit()
            } else {
                if( entity == null) {
    ...
    private fun onCreateNewItem() {
        ...
        val fragment = INREPRINTPOSetCreateFragment()
    ...
    @Suppress("UNUSED_PARAMETER")
    private fun onEditItem(entity: InReprintPo?) {
        val updateFragment = INREPRINTPOSetCreateFragment()
        val arguments = Bundle()
    ...
    internal class ConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            builder.setPositiveButton(R.string.before_navigation_dialog_positive_button) { _, _ ->
                (fragmentActivity as INREPRINTPOSetActivity).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null)
            }
    ...
    internal class DeleteConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            val fragmentActivity = requireActivity() as AppCompatActivity
            val builder = MaterialAlertDialogBuilder(fragmentActivity, R.style.AlertDialogStyle)
            val viewModel = ViewModelProvider(fragmentActivity)[InReprintPoViewModel::class.java]
