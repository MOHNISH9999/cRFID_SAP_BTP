...
import com.company.crfid_sap_btp.databinding.FragmentOutpickingsetDetailBinding
...
class OUTPICKINGSetDetailFragment : InterfacedFragment<OutPicking, FragmentOutpickingsetDetailBinding>() {
    ...
    private fun setDetailImage(objectHeader: ObjectHeader, outPickingEntity: OutPicking) {
        if (outPickingEntity.getOptionalValue(OutPicking.name1) != null && !outPickingEntity.getOptionalValue(OutPicking.name1).toString().isEmpty()) {
