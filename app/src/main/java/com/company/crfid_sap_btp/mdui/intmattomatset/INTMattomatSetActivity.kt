 ...
 * - [INTMattomatSetCreateFragment]: to create or edit a(n) INTMattomat,
 * - [INTMattomatSetDetailFragment]: to display the details of a(n) INTMattomat,
 * - [INTMattomatSetListFragment]: to list INTMattomatSet.
 *
 ...
 * what to do.
 */
class INTMattomatSetActivity : AppCompatActivity(), InterfacedFragment.InterfacedFragmentListener<INTMattomat> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, INTMattomatSetListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit()
        } else {
    ...
    override fun onFragmentStateChange(evt: Int, entity: INTMattomat?) {
        when (evt) {
    ...
    /**
     * Shows an AlertDialog when user wants to leave {@link INTMattomatSetCreateFragment}.
     */
    ...
    private fun onItemClicked(entity: INTMattomat?) {
        if (!isMasterDetailView) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, INTMattomatSetDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if (detail == null) {
                if (entity != null)
                    supportFragmentManager.beginTransaction()
                        .replace(R.id.detailFrame, INTMattomatSetDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                        .commit()
            } else {
                if( entity == null) {
    ...
    private fun onCreateNewItem() {
        ...
        val fragment = INTMattomatSetCreateFragment()
    ...
    @Suppress("UNUSED_PARAMETER")
    private fun onEditItem(entity: INTMattomat?) {
        val updateFragment = INTMattomatSetCreateFragment()
        val arguments = Bundle()
    ...
    internal class ConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            builder.setPositiveButton(R.string.before_navigation_dialog_positive_button) { _, _ ->
                (fragmentActivity as INTMattomatSetActivity).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null)
            }
    ...
    internal class DeleteConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            val fragmentActivity = requireActivity() as AppCompatActivity
            val builder = MaterialAlertDialogBuilder(fragmentActivity, R.style.AlertDialogStyle)
            val viewModel = ViewModelProvider(fragmentActivity)[INTMattomatViewModel::class.java]
