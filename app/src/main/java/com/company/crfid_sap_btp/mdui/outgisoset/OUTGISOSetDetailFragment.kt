...
import com.company.crfid_sap_btp.databinding.FragmentOutgisosetDetailBinding
...
class OUTGISOSetDetailFragment : InterfacedFragment<OutGiSo, FragmentOutgisosetDetailBinding>() {
    ...
    private fun setDetailImage(objectHeader: ObjectHeader, outGiSoEntity: OutGiSo) {
        if (outGiSoEntity.getOptionalValue(OutGiSo.vbtyp) != null && !outGiSoEntity.getOptionalValue(OutGiSo.vbtyp).toString().isEmpty()) {
