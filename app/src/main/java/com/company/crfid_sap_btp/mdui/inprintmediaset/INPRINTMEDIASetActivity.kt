 ...
 * - [INPRINTMEDIASetCreateFragment]: to create or edit a(n) InPrintMedia,
 * - [INPRINTMEDIASetDetailFragment]: to display the details of a(n) InPrintMedia,
 * - [INPRINTMEDIASetListFragment]: to list INPRINTMEDIASet.
 *
 ...
 * what to do.
 */
class INPRINTMEDIASetActivity : AppCompatActivity(), InterfacedFragment.InterfacedFragmentListener<InPrintMedia> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, INPRINTMEDIASetListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit()
        } else {
    ...
    override fun onFragmentStateChange(evt: Int, entity: InPrintMedia?) {
        when (evt) {
    ...
    /**
     * Shows an AlertDialog when user wants to leave {@link INPRINTMEDIASetCreateFragment}.
     */
    ...
    private fun onItemClicked(entity: InPrintMedia?) {
        if (!isMasterDetailView) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, INPRINTMEDIASetDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if (detail == null) {
                if (entity != null)
                    supportFragmentManager.beginTransaction()
                        .replace(R.id.detailFrame, INPRINTMEDIASetDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                        .commit()
            } else {
                if( entity == null) {
    ...
    private fun onCreateNewItem() {
        ...
        val fragment = INPRINTMEDIASetCreateFragment()
    ...
    @Suppress("UNUSED_PARAMETER")
    private fun onEditItem(entity: InPrintMedia?) {
        val updateFragment = INPRINTMEDIASetCreateFragment()
        val arguments = Bundle()
    ...
    internal class ConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            builder.setPositiveButton(R.string.before_navigation_dialog_positive_button) { _, _ ->
                (fragmentActivity as INPRINTMEDIASetActivity).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null)
            }
    ...
    internal class DeleteConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            val fragmentActivity = requireActivity() as AppCompatActivity
            val builder = MaterialAlertDialogBuilder(fragmentActivity, R.style.AlertDialogStyle)
            val viewModel = ViewModelProvider(fragmentActivity)[InPrintMediaViewModel::class.java]
