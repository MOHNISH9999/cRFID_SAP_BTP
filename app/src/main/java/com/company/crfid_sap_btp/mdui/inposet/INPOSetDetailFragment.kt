...
import com.company.crfid_sap_btp.databinding.FragmentInposetDetailBinding
...
class INPOSetDetailFragment : InterfacedFragment<InPo, FragmentInposetDetailBinding>() {
    ...
    private fun setDetailImage(objectHeader: ObjectHeader, inPoEntity: InPo) {
        if (inPoEntity.getOptionalValue(InPo.bsart) != null && !inPoEntity.getOptionalValue(InPo.bsart).toString().isEmpty()) {
