...
import com.company.crfid_sap_btp.databinding.FragmentInpoitemsetDetailBinding
...
class INPOITEMSetDetailFragment : InterfacedFragment<InPoItem, FragmentInpoitemsetDetailBinding>() {
    ...
    private fun setDetailImage(objectHeader: ObjectHeader, inPoItemEntity: InPoItem) {
        if (inPoItemEntity.getOptionalValue(InPoItem.ebelp) != null && !inPoItemEntity.getOptionalValue(InPoItem.ebelp).toString().isEmpty()) {
