 ...
 * - [OUTGISCRAPSetCreateFragment]: to create or edit a(n) OutGiScrap,
 * - [OUTGISCRAPSetDetailFragment]: to display the details of a(n) OutGiScrap,
 * - [OUTGISCRAPSetListFragment]: to list OUTGISCRAPSet.
 *
 ...
 * what to do.
 */
class OUTGISCRAPSetActivity : AppCompatActivity(), InterfacedFragment.InterfacedFragmentListener<OutGiScrap> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, OUTGISCRAPSetListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit()
        } else {
    ...
    override fun onFragmentStateChange(evt: Int, entity: OutGiScrap?) {
        when (evt) {
    ...
    /**
     * Shows an AlertDialog when user wants to leave {@link OUTGISCRAPSetCreateFragment}.
     */
    ...
    private fun onItemClicked(entity: OutGiScrap?) {
        if (!isMasterDetailView) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, OUTGISCRAPSetDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if (detail == null) {
                if (entity != null)
                    supportFragmentManager.beginTransaction()
                        .replace(R.id.detailFrame, OUTGISCRAPSetDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                        .commit()
            } else {
                if( entity == null) {
    ...
    private fun onCreateNewItem() {
        ...
        val fragment = OUTGISCRAPSetCreateFragment()
    ...
    @Suppress("UNUSED_PARAMETER")
    private fun onEditItem(entity: OutGiScrap?) {
        val updateFragment = OUTGISCRAPSetCreateFragment()
        val arguments = Bundle()
    ...
    internal class ConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            builder.setPositiveButton(R.string.before_navigation_dialog_positive_button) { _, _ ->
                (fragmentActivity as OUTGISCRAPSetActivity).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null)
            }
    ...
    internal class DeleteConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            val fragmentActivity = requireActivity() as AppCompatActivity
            val builder = MaterialAlertDialogBuilder(fragmentActivity, R.style.AlertDialogStyle)
            val viewModel = ViewModelProvider(fragmentActivity)[OutGiScrapViewModel::class.java]
