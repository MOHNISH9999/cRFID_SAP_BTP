package com.company.crfid_sap_btp.mdui.outgireservationset
 ...
 * - [OutGIReservationSetCreateFragment]: to create or edit a(n) OutGIReservation,
 * - [OutGIReservationSetDetailFragment]: to display the details of a(n) OutGIReservation,
 * - [OutGIReservationSetListFragment]: to list OutGIReservationSet.
 *
 ...
 * what to do.
 */
class OutGIReservationSetActivity : AppCompatActivity(), InterfacedFragment.InterfacedFragmentListener<OutGIReservation> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, OutGIReservationSetListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit()
        } else {
    ...
    override fun onFragmentStateChange(evt: Int, entity: OutGIReservation?) {
        when (evt) {
    ...
    /**
     * Shows an AlertDialog when user wants to leave {@link OutGIReservationSetCreateFragment}.
     */
    ...
    private fun onItemClicked(entity: OutGIReservation?) {
        if (!isMasterDetailView) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, OutGIReservationSetDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if (detail == null) {
                if (entity != null)
                    supportFragmentManager.beginTransaction()
                        .replace(R.id.detailFrame, OutGIReservationSetDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                        .commit()
            } else {
                if( entity == null) {
    ...
    private fun onCreateNewItem() {
        ...
        val fragment = OutGIReservationSetCreateFragment()
    ...
    @Suppress("UNUSED_PARAMETER")
    private fun onEditItem(entity: OutGIReservation?) {
        val updateFragment = OutGIReservationSetCreateFragment()
        val arguments = Bundle()
    ...
    internal class ConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            builder.setPositiveButton(R.string.before_navigation_dialog_positive_button) { _, _ ->
                (fragmentActivity as OutGIReservationSetActivity).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null)
            }
    ...
    internal class DeleteConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            val fragmentActivity = requireActivity() as AppCompatActivity
            val builder = MaterialAlertDialogBuilder(fragmentActivity, R.style.AlertDialogStyle)
            val viewModel = ViewModelProvider(fragmentActivity)[OutGIReservationViewModel::class.java]
