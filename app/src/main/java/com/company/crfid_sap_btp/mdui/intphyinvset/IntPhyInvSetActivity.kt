 ...
 * - [IntPhyInvSetCreateFragment]: to create or edit a(n) IntPhyInv,
 * - [IntPhyInvSetDetailFragment]: to display the details of a(n) IntPhyInv,
 * - [IntPhyInvSetListFragment]: to list IntPhyInvSet.
 *
 ...
 * what to do.
 */
class IntPhyInvSetActivity : AppCompatActivity(), InterfacedFragment.InterfacedFragmentListener<IntPhyInv> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, IntPhyInvSetListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit()
        } else {
    ...
    override fun onFragmentStateChange(evt: Int, entity: IntPhyInv?) {
        when (evt) {
    ...
    /**
     * Shows an AlertDialog when user wants to leave {@link IntPhyInvSetCreateFragment}.
     */
    ...
    private fun onItemClicked(entity: IntPhyInv?) {
        if (!isMasterDetailView) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, IntPhyInvSetDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if (detail == null) {
                if (entity != null)
                    supportFragmentManager.beginTransaction()
                        .replace(R.id.detailFrame, IntPhyInvSetDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                        .commit()
            } else {
                if( entity == null) {
    ...
    private fun onCreateNewItem() {
        ...
        val fragment = IntPhyInvSetCreateFragment()
    ...
    @Suppress("UNUSED_PARAMETER")
    private fun onEditItem(entity: IntPhyInv?) {
        val updateFragment = IntPhyInvSetCreateFragment()
        val arguments = Bundle()
    ...
    internal class ConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            builder.setPositiveButton(R.string.before_navigation_dialog_positive_button) { _, _ ->
                (fragmentActivity as IntPhyInvSetActivity).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null)
            }
    ...
    internal class DeleteConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            val fragmentActivity = requireActivity() as AppCompatActivity
            val builder = MaterialAlertDialogBuilder(fragmentActivity, R.style.AlertDialogStyle)
            val viewModel = ViewModelProvider(fragmentActivity)[IntPhyInvViewModel::class.java]
