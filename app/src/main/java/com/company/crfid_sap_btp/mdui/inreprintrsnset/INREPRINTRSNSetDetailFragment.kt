...
import com.company.crfid_sap_btp.databinding.FragmentInreprintrsnsetDetailBinding
...
class INREPRINTRSNSetDetailFragment : InterfacedFragment<InReprintRsn, FragmentInreprintrsnsetDetailBinding>() {
    ...
    private fun setDetailImage(objectHeader: ObjectHeader, inReprintRsnEntity: InReprintRsn) {
        if (inReprintRsnEntity.getOptionalValue(InReprintRsn.rreason) != null && !inReprintRsnEntity.getOptionalValue(InReprintRsn.rreason).toString().isEmpty()) {
