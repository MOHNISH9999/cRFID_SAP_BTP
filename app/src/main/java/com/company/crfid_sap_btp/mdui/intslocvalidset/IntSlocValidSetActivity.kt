package com.company.crfid_sap_btp.mdui.intslocvalidset
 ...
 * - [IntSlocValidSetCreateFragment]: to create or edit a(n) IntSlocValid,
 * - [IntSlocValidSetDetailFragment]: to display the details of a(n) IntSlocValid,
 * - [IntSlocValidSetListFragment]: to list IntSlocValidSet.
 *
 ...
 * what to do.
 */
class IntSlocValidSetActivity : AppCompatActivity(), InterfacedFragment.InterfacedFragmentListener<IntSlocValid> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, IntSlocValidSetListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit()
        } else {
    ...
    override fun onFragmentStateChange(evt: Int, entity: IntSlocValid?) {
        when (evt) {
    ...
    /**
     * Shows an AlertDialog when user wants to leave {@link IntSlocValidSetCreateFragment}.
     */
    ...
    private fun onItemClicked(entity: IntSlocValid?) {
        if (!isMasterDetailView) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, IntSlocValidSetDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if (detail == null) {
                if (entity != null)
                    supportFragmentManager.beginTransaction()
                        .replace(R.id.detailFrame, IntSlocValidSetDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                        .commit()
            } else {
                if( entity == null) {
    ...
    private fun onCreateNewItem() {
        ...
        val fragment = IntSlocValidSetCreateFragment()
    ...
    @Suppress("UNUSED_PARAMETER")
    private fun onEditItem(entity: IntSlocValid?) {
        val updateFragment = IntSlocValidSetCreateFragment()
        val arguments = Bundle()
    ...
    internal class ConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            builder.setPositiveButton(R.string.before_navigation_dialog_positive_button) { _, _ ->
                (fragmentActivity as IntSlocValidSetActivity).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null)
            }
    ...
    internal class DeleteConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            val fragmentActivity = requireActivity() as AppCompatActivity
            val builder = MaterialAlertDialogBuilder(fragmentActivity, R.style.AlertDialogStyle)
            val viewModel = ViewModelProvider(fragmentActivity)[IntSlocValidViewModel::class.java]
