 ...
 * - [OUTPICKING1SetCreateFragment]: to create or edit a(n) OutPicking1,
 * - [OUTPICKING1SetDetailFragment]: to display the details of a(n) OutPicking1,
 * - [OUTPICKING1SetListFragment]: to list OUTPICKING1Set.
 *
 ...
 * what to do.
 */
class OUTPICKING1SetActivity : AppCompatActivity(), InterfacedFragment.InterfacedFragmentListener<OutPicking1> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, OUTPICKING1SetListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit()
        } else {
    ...
    override fun onFragmentStateChange(evt: Int, entity: OutPicking1?) {
        when (evt) {
    ...
    /**
     * Shows an AlertDialog when user wants to leave {@link OUTPICKING1SetCreateFragment}.
     */
    ...
    private fun onItemClicked(entity: OutPicking1?) {
        if (!isMasterDetailView) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, OUTPICKING1SetDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if (detail == null) {
                if (entity != null)
                    supportFragmentManager.beginTransaction()
                        .replace(R.id.detailFrame, OUTPICKING1SetDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                        .commit()
            } else {
                if( entity == null) {
    ...
    private fun onCreateNewItem() {
        ...
        val fragment = OUTPICKING1SetCreateFragment()
    ...
    @Suppress("UNUSED_PARAMETER")
    private fun onEditItem(entity: OutPicking1?) {
        val updateFragment = OUTPICKING1SetCreateFragment()
        val arguments = Bundle()
    ...
    internal class ConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            builder.setPositiveButton(R.string.before_navigation_dialog_positive_button) { _, _ ->
                (fragmentActivity as OUTPICKING1SetActivity).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null)
            }
    ...
    internal class DeleteConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            val fragmentActivity = requireActivity() as AppCompatActivity
            val builder = MaterialAlertDialogBuilder(fragmentActivity, R.style.AlertDialogStyle)
            val viewModel = ViewModelProvider(fragmentActivity)[OutPicking1ViewModel::class.java]
