package com.company.crfid_sap_btp.mdui.intmtomchargvalidset
...
import com.company.crfid_sap_btp.databinding.ActivityEntityitemBinding
 ...
 * - [INTMtomChargvalidSetCreateFragment]: to create or edit a(n) INTMtomChargvalid,
 * - [INTMtomChargvalidSetDetailFragment]: to display the details of a(n) INTMtomChargvalid,
 * - [INTMtomChargvalidSetListFragment]: to list INTMtomChargvalidSet.
 *
 ...
 * what to do.
 */
class INTMtomChargvalidSetActivity : AppCompatActivity(), InterfacedFragment.InterfacedFragmentListener<INTMtomChargvalid> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, INTMtomChargvalidSetListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit()
        } else {
    ...
    override fun onFragmentStateChange(evt: Int, entity: INTMtomChargvalid?) {
        when (evt) {
    ...
    /**
     * Shows an AlertDialog when user wants to leave {@link INTMtomChargvalidSetCreateFragment}.
     */
    ...
    private fun onItemClicked(entity: INTMtomChargvalid?) {
        if (!isMasterDetailView) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, INTMtomChargvalidSetDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if (detail == null) {
                if (entity != null)
                    supportFragmentManager.beginTransaction()
                        .replace(R.id.detailFrame, INTMtomChargvalidSetDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                        .commit()
            } else {
                if( entity == null) {
    ...
    private fun onCreateNewItem() {
        ...
        val fragment = INTMtomChargvalidSetCreateFragment()
    ...
    @Suppress("UNUSED_PARAMETER")
    private fun onEditItem(entity: INTMtomChargvalid?) {
        val updateFragment = INTMtomChargvalidSetCreateFragment()
        val arguments = Bundle()
    ...
    internal class ConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            builder.setPositiveButton(R.string.before_navigation_dialog_positive_button) { _, _ ->
                (fragmentActivity as INTMtomChargvalidSetActivity).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null)
            }
    ...
    internal class DeleteConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            val fragmentActivity = requireActivity() as AppCompatActivity
            val builder = MaterialAlertDialogBuilder(fragmentActivity, R.style.AlertDialogStyle)
            val viewModel = ViewModelProvider(fragmentActivity)[INTMtomChargvalidViewModel::class.java]
