...
import com.company.crfid_sap_btp.databinding.FragmentRfmattagsetDetailBinding
...
class RFMATTAGSetDetailFragment : InterfacedFragment<RfMattag, FragmentRfmattagsetDetailBinding>() {
    ...
    private fun setDetailImage(objectHeader: ObjectHeader, rfMattagEntity: RfMattag) {
        if (rfMattagEntity.getOptionalValue(RfMattag.werks) != null && !rfMattagEntity.getOptionalValue(RfMattag.werks).toString().isEmpty()) {
