...
import com.company.crfid_sap_btp.databinding.FragmentInpoheadersetDetailBinding
...
class INPOHEADERSetDetailFragment : InterfacedFragment<InPoHeader, FragmentInpoheadersetDetailBinding>() {
    ...
    private fun setDetailImage(objectHeader: ObjectHeader, inPoHeaderEntity: InPoHeader) {
        if (inPoHeaderEntity.getOptionalValue(InPoHeader.ebelp) != null && !inPoHeaderEntity.getOptionalValue(InPoHeader.ebelp).toString().isEmpty()) {
