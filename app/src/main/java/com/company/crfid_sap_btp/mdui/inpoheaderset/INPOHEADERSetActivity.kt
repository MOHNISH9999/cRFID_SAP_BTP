 ...
 * - [INPOHEADERSetCreateFragment]: to create or edit a(n) InPoHeader,
 * - [INPOHEADERSetDetailFragment]: to display the details of a(n) InPoHeader,
 * - [INPOHEADERSetListFragment]: to list INPOHEADERSet.
 *
 ...
 * what to do.
 */
class INPOHEADERSetActivity : AppCompatActivity(), InterfacedFragment.InterfacedFragmentListener<InPoHeader> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, INPOHEADERSetListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit()
        } else {
    ...
    override fun onFragmentStateChange(evt: Int, entity: InPoHeader?) {
        when (evt) {
    ...
    /**
     * Shows an AlertDialog when user wants to leave {@link INPOHEADERSetCreateFragment}.
     */
    ...
    private fun onItemClicked(entity: InPoHeader?) {
        if (!isMasterDetailView) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, INPOHEADERSetDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if (detail == null) {
                if (entity != null)
                    supportFragmentManager.beginTransaction()
                        .replace(R.id.detailFrame, INPOHEADERSetDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                        .commit()
            } else {
                if( entity == null) {
    ...
    private fun onCreateNewItem() {
        ...
        val fragment = INPOHEADERSetCreateFragment()
    ...
    @Suppress("UNUSED_PARAMETER")
    private fun onEditItem(entity: InPoHeader?) {
        val updateFragment = INPOHEADERSetCreateFragment()
        val arguments = Bundle()
    ...
    internal class ConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            builder.setPositiveButton(R.string.before_navigation_dialog_positive_button) { _, _ ->
                (fragmentActivity as INPOHEADERSetActivity).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null)
            }
    ...
    internal class DeleteConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            val fragmentActivity = requireActivity() as AppCompatActivity
            val builder = MaterialAlertDialogBuilder(fragmentActivity, R.style.AlertDialogStyle)
            val viewModel = ViewModelProvider(fragmentActivity)[InPoHeaderViewModel::class.java]
