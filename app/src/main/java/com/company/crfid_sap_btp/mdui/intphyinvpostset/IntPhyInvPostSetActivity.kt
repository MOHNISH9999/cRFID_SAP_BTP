 ...
 * - [IntPhyInvPostSetCreateFragment]: to create or edit a(n) IntPhyInvPost,
 * - [IntPhyInvPostSetDetailFragment]: to display the details of a(n) IntPhyInvPost,
 * - [IntPhyInvPostSetListFragment]: to list IntPhyInvPostSet.
 *
 ...
 * what to do.
 */
class IntPhyInvPostSetActivity : AppCompatActivity(), InterfacedFragment.InterfacedFragmentListener<IntPhyInvPost> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, IntPhyInvPostSetListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit()
        } else {
    ...
    override fun onFragmentStateChange(evt: Int, entity: IntPhyInvPost?) {
        when (evt) {
    ...
    /**
     * Shows an AlertDialog when user wants to leave {@link IntPhyInvPostSetCreateFragment}.
     */
    ...
    private fun onItemClicked(entity: IntPhyInvPost?) {
        if (!isMasterDetailView) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, IntPhyInvPostSetDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if (detail == null) {
                if (entity != null)
                    supportFragmentManager.beginTransaction()
                        .replace(R.id.detailFrame, IntPhyInvPostSetDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                        .commit()
            } else {
                if( entity == null) {
    ...
    private fun onCreateNewItem() {
        ...
        val fragment = IntPhyInvPostSetCreateFragment()
    ...
    @Suppress("UNUSED_PARAMETER")
    private fun onEditItem(entity: IntPhyInvPost?) {
        val updateFragment = IntPhyInvPostSetCreateFragment()
        val arguments = Bundle()
    ...
    internal class ConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            builder.setPositiveButton(R.string.before_navigation_dialog_positive_button) { _, _ ->
                (fragmentActivity as IntPhyInvPostSetActivity).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null)
            }
    ...
    internal class DeleteConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            val fragmentActivity = requireActivity() as AppCompatActivity
            val builder = MaterialAlertDialogBuilder(fragmentActivity, R.style.AlertDialogStyle)
            val viewModel = ViewModelProvider(fragmentActivity)[IntPhyInvPostViewModel::class.java]
