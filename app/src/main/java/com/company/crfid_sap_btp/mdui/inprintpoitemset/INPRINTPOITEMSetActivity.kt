 ...
 * - [INPRINTPOITEMSetCreateFragment]: to create or edit a(n) InPrintPoItem,
 * - [INPRINTPOITEMSetDetailFragment]: to display the details of a(n) InPrintPoItem,
 * - [INPRINTPOITEMSetListFragment]: to list INPRINTPOITEMSet.
 *
 ...
 * what to do.
 */
class INPRINTPOITEMSetActivity : AppCompatActivity(), InterfacedFragment.InterfacedFragmentListener<InPrintPoItem> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, INPRINTPOITEMSetListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit()
        } else {
    ...
    override fun onFragmentStateChange(evt: Int, entity: InPrintPoItem?) {
        when (evt) {
    ...
    /**
     * Shows an AlertDialog when user wants to leave {@link INPRINTPOITEMSetCreateFragment}.
     */
    ...
    private fun onItemClicked(entity: InPrintPoItem?) {
        if (!isMasterDetailView) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, INPRINTPOITEMSetDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if (detail == null) {
                if (entity != null)
                    supportFragmentManager.beginTransaction()
                        .replace(R.id.detailFrame, INPRINTPOITEMSetDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                        .commit()
            } else {
                if( entity == null) {
    ...
    private fun onCreateNewItem() {
        ...
        val fragment = INPRINTPOITEMSetCreateFragment()
    ...
    @Suppress("UNUSED_PARAMETER")
    private fun onEditItem(entity: InPrintPoItem?) {
        val updateFragment = INPRINTPOITEMSetCreateFragment()
        val arguments = Bundle()
    ...
    internal class ConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            builder.setPositiveButton(R.string.before_navigation_dialog_positive_button) { _, _ ->
                (fragmentActivity as INPRINTPOITEMSetActivity).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null)
            }
    ...
    internal class DeleteConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            val fragmentActivity = requireActivity() as AppCompatActivity
            val builder = MaterialAlertDialogBuilder(fragmentActivity, R.style.AlertDialogStyle)
            val viewModel = ViewModelProvider(fragmentActivity)[InPrintPoItemViewModel::class.java]
