 ...
 * - [IntPhyValidSetCreateFragment]: to create or edit a(n) IntPhyValid,
 * - [IntPhyValidSetDetailFragment]: to display the details of a(n) IntPhyValid,
 * - [IntPhyValidSetListFragment]: to list IntPhyValidSet.
 *
 ...
 * what to do.
 */
class IntPhyValidSetActivity : AppCompatActivity(), InterfacedFragment.InterfacedFragmentListener<IntPhyValid> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, IntPhyValidSetListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit()
        } else {
    ...
    override fun onFragmentStateChange(evt: Int, entity: IntPhyValid?) {
        when (evt) {
    ...
    /**
     * Shows an AlertDialog when user wants to leave {@link IntPhyValidSetCreateFragment}.
     */
    ...
    private fun onItemClicked(entity: IntPhyValid?) {
        if (!isMasterDetailView) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, IntPhyValidSetDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if (detail == null) {
                if (entity != null)
                    supportFragmentManager.beginTransaction()
                        .replace(R.id.detailFrame, IntPhyValidSetDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                        .commit()
            } else {
                if( entity == null) {
    ...
    private fun onCreateNewItem() {
        ...
        val fragment = IntPhyValidSetCreateFragment()
    ...
    @Suppress("UNUSED_PARAMETER")
    private fun onEditItem(entity: IntPhyValid?) {
        val updateFragment = IntPhyValidSetCreateFragment()
        val arguments = Bundle()
    ...
    internal class ConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            builder.setPositiveButton(R.string.before_navigation_dialog_positive_button) { _, _ ->
                (fragmentActivity as IntPhyValidSetActivity).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null)
            }
    ...
    internal class DeleteConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            val fragmentActivity = requireActivity() as AppCompatActivity
            val builder = MaterialAlertDialogBuilder(fragmentActivity, R.style.AlertDialogStyle)
            val viewModel = ViewModelProvider(fragmentActivity)[IntPhyValidViewModel::class.java]
