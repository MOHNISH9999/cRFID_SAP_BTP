package com.company.crfid_sap_btp.mdui.outgisoset
 ...
 * - [OUTGISOSetCreateFragment]: to create or edit a(n) OutGiSo,
 * - [OUTGISOSetDetailFragment]: to display the details of a(n) OutGiSo,
 * - [OUTGISOSetListFragment]: to list OUTGISOSet.
 *
 ...
 * what to do.
 */
class OUTGISOSetActivity : AppCompatActivity(), InterfacedFragment.InterfacedFragmentListener<OutGiSo> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, OUTGISOSetListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit()
        } else {
    ...
    override fun onFragmentStateChange(evt: Int, entity: OutGiSo?) {
        when (evt) {
    ...
    /**
     * Shows an AlertDialog when user wants to leave {@link OUTGISOSetCreateFragment}.
     */
    ...
    private fun onItemClicked(entity: OutGiSo?) {
        if (!isMasterDetailView) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, OUTGISOSetDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if (detail == null) {
                if (entity != null)
                    supportFragmentManager.beginTransaction()
                        .replace(R.id.detailFrame, OUTGISOSetDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                        .commit()
            } else {
                if( entity == null) {
    ...
    private fun onCreateNewItem() {
        ...
        val fragment = OUTGISOSetCreateFragment()
    ...
    @Suppress("UNUSED_PARAMETER")
    private fun onEditItem(entity: OutGiSo?) {
        val updateFragment = OUTGISOSetCreateFragment()
        val arguments = Bundle()
    ...
    internal class ConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            builder.setPositiveButton(R.string.before_navigation_dialog_positive_button) { _, _ ->
                (fragmentActivity as OUTGISOSetActivity).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null)
            }
    ...
    internal class DeleteConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            val fragmentActivity = requireActivity() as AppCompatActivity
            val builder = MaterialAlertDialogBuilder(fragmentActivity, R.style.AlertDialogStyle)
            val viewModel = ViewModelProvider(fragmentActivity)[OutGiSoViewModel::class.java]
