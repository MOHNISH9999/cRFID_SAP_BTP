package com.company.crfid_sap_btp.mdui.outpickingset
 ...
 * - [OUTPICKINGSetCreateFragment]: to create or edit a(n) OutPicking,
 * - [OUTPICKINGSetDetailFragment]: to display the details of a(n) OutPicking,
 * - [OUTPICKINGSetListFragment]: to list OUTPICKINGSet.
 *
 ...
 * what to do.
 */
class OUTPICKINGSetActivity : AppCompatActivity(), InterfacedFragment.InterfacedFragmentListener<OutPicking> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, OUTPICKINGSetListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit()
        } else {
    ...
    override fun onFragmentStateChange(evt: Int, entity: OutPicking?) {
        when (evt) {
    ...
    /**
     * Shows an AlertDialog when user wants to leave {@link OUTPICKINGSetCreateFragment}.
     */
    ...
    private fun onItemClicked(entity: OutPicking?) {
        if (!isMasterDetailView) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, OUTPICKINGSetDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if (detail == null) {
                if (entity != null)
                    supportFragmentManager.beginTransaction()
                        .replace(R.id.detailFrame, OUTPICKINGSetDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                        .commit()
            } else {
                if( entity == null) {
    ...
    private fun onCreateNewItem() {
        ...
        val fragment = OUTPICKINGSetCreateFragment()
    ...
    @Suppress("UNUSED_PARAMETER")
    private fun onEditItem(entity: OutPicking?) {
        val updateFragment = OUTPICKINGSetCreateFragment()
        val arguments = Bundle()
    ...
    internal class ConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            builder.setPositiveButton(R.string.before_navigation_dialog_positive_button) { _, _ ->
                (fragmentActivity as OUTPICKINGSetActivity).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null)
            }
    ...
    internal class DeleteConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            val fragmentActivity = requireActivity() as AppCompatActivity
            val builder = MaterialAlertDialogBuilder(fragmentActivity, R.style.AlertDialogStyle)
            val viewModel = ViewModelProvider(fragmentActivity)[OutPickingViewModel::class.java]
