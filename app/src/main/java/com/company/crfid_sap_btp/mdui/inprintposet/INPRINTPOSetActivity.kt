 ...
 * - [INPRINTPOSetCreateFragment]: to create or edit a(n) InPrintPo,
 * - [INPRINTPOSetDetailFragment]: to display the details of a(n) InPrintPo,
 * - [INPRINTPOSetListFragment]: to list INPRINTPOSet.
 *
 ...
 * what to do.
 */
class INPRINTPOSetActivity : AppCompatActivity(), InterfacedFragment.InterfacedFragmentListener<InPrintPo> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, INPRINTPOSetListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit()
        } else {
    ...
    override fun onFragmentStateChange(evt: Int, entity: InPrintPo?) {
        when (evt) {
    ...
    /**
     * Shows an AlertDialog when user wants to leave {@link INPRINTPOSetCreateFragment}.
     */
    ...
    private fun onItemClicked(entity: InPrintPo?) {
        if (!isMasterDetailView) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, INPRINTPOSetDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if (detail == null) {
                if (entity != null)
                    supportFragmentManager.beginTransaction()
                        .replace(R.id.detailFrame, INPRINTPOSetDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                        .commit()
            } else {
                if( entity == null) {
    ...
    private fun onCreateNewItem() {
        ...
        val fragment = INPRINTPOSetCreateFragment()
    ...
    @Suppress("UNUSED_PARAMETER")
    private fun onEditItem(entity: InPrintPo?) {
        val updateFragment = INPRINTPOSetCreateFragment()
        val arguments = Bundle()
    ...
    internal class ConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            builder.setPositiveButton(R.string.before_navigation_dialog_positive_button) { _, _ ->
                (fragmentActivity as INPRINTPOSetActivity).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null)
            }
    ...
    internal class DeleteConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            val fragmentActivity = requireActivity() as AppCompatActivity
            val builder = MaterialAlertDialogBuilder(fragmentActivity, R.style.AlertDialogStyle)
            val viewModel = ViewModelProvider(fragmentActivity)[InPrintPoViewModel::class.java]
