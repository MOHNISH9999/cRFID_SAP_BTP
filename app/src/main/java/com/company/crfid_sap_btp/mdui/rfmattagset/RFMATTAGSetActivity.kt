package com.company.crfid_sap_btp.mdui.rfmattagset
 ...
 * - [RFMATTAGSetCreateFragment]: to create or edit a(n) RfMattag,
 * - [RFMATTAGSetDetailFragment]: to display the details of a(n) RfMattag,
 * - [RFMATTAGSetListFragment]: to list RFMATTAGSet.
 *
 ...
 * what to do.
 */
class RFMATTAGSetActivity : AppCompatActivity(), InterfacedFragment.InterfacedFragmentListener<RfMattag> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, RFMATTAGSetListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit()
        } else {
    ...
    override fun onFragmentStateChange(evt: Int, entity: RfMattag?) {
        when (evt) {
    ...
    /**
     * Shows an AlertDialog when user wants to leave {@link RFMATTAGSetCreateFragment}.
     */
    ...
    private fun onItemClicked(entity: RfMattag?) {
        if (!isMasterDetailView) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, RFMATTAGSetDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if (detail == null) {
                if (entity != null)
                    supportFragmentManager.beginTransaction()
                        .replace(R.id.detailFrame, RFMATTAGSetDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                        .commit()
            } else {
                if( entity == null) {
    ...
    private fun onCreateNewItem() {
        ...
        val fragment = RFMATTAGSetCreateFragment()
    ...
    @Suppress("UNUSED_PARAMETER")
    private fun onEditItem(entity: RfMattag?) {
        val updateFragment = RFMATTAGSetCreateFragment()
        val arguments = Bundle()
    ...
    internal class ConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            builder.setPositiveButton(R.string.before_navigation_dialog_positive_button) { _, _ ->
                (fragmentActivity as RFMATTAGSetActivity).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null)
            }
    ...
    internal class DeleteConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            val fragmentActivity = requireActivity() as AppCompatActivity
            val builder = MaterialAlertDialogBuilder(fragmentActivity, R.style.AlertDialogStyle)
            val viewModel = ViewModelProvider(fragmentActivity)[RfMattagViewModel::class.java]
