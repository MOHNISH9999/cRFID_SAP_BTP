 ...
 * - [IntSlocToSlocSetCreateFragment]: to create or edit a(n) IntSlocToSloc,
 * - [IntSlocToSlocSetDetailFragment]: to display the details of a(n) IntSlocToSloc,
 * - [IntSlocToSlocSetListFragment]: to list IntSlocToSlocSet.
 *
 ...
 * what to do.
 */
class IntSlocToSlocSetActivity : AppCompatActivity(), InterfacedFragment.InterfacedFragmentListener<IntSlocToSloc> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, IntSlocToSlocSetListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit()
        } else {
    ...
    override fun onFragmentStateChange(evt: Int, entity: IntSlocToSloc?) {
        when (evt) {
    ...
    /**
     * Shows an AlertDialog when user wants to leave {@link IntSlocToSlocSetCreateFragment}.
     */
    ...
    private fun onItemClicked(entity: IntSlocToSloc?) {
        if (!isMasterDetailView) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, IntSlocToSlocSetDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if (detail == null) {
                if (entity != null)
                    supportFragmentManager.beginTransaction()
                        .replace(R.id.detailFrame, IntSlocToSlocSetDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                        .commit()
            } else {
                if( entity == null) {
    ...
    private fun onCreateNewItem() {
        ...
        val fragment = IntSlocToSlocSetCreateFragment()
    ...
    @Suppress("UNUSED_PARAMETER")
    private fun onEditItem(entity: IntSlocToSloc?) {
        val updateFragment = IntSlocToSlocSetCreateFragment()
        val arguments = Bundle()
    ...
    internal class ConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            builder.setPositiveButton(R.string.before_navigation_dialog_positive_button) { _, _ ->
                (fragmentActivity as IntSlocToSlocSetActivity).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null)
            }
    ...
    internal class DeleteConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            val fragmentActivity = requireActivity() as AppCompatActivity
            val builder = MaterialAlertDialogBuilder(fragmentActivity, R.style.AlertDialogStyle)
            val viewModel = ViewModelProvider(fragmentActivity)[IntSlocToSlocViewModel::class.java]
